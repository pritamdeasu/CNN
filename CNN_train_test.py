...
class Net:
    ...
    #after training the model, we know the weight, so we will do forward pass only
...
"""
print('Loading data......')
sub_train_images, sub_train_labels, sub_test_images, sub_test_labels = init_subset('4526') # input your ASUID last 4 digits here to generate the subset samples of MNIST for training and testing
...
def evaluate(net, images, labels):
    ...
    return (acc/train_size)*100, (loss/train_size)*100

def evaluate_test(net,images, labels):
    acc = 0    
    loss = 0
    

    n=images.shape[0]
    loss,acc=net.test(images,labels)
    return (acc/images.shape[0])*100,(loss/images.shape[0])*100
...
for e in range(epoch):
    ...
    train_acc, train_loss = evaluate_test(net, sub_train_images, sub_train_labels)  # Use the evaluation code to obtain the training accuracy and loss
    test_acc, test_loss = evaluate_test(net, sub_test_images, sub_test_labels)      # Use the evaluation code to obtain the testing accuracy and loss
...
df=pd.concat([df,pd.DataFrame(train_loss_list)],axis=1)
df=df.rename(columns={0:'Train_Loss'})

df=pd.concat([df,pd.DataFrame(test_loss_list)],axis=1)
df=df.rename(columns={0:'Test_Loss'})

plt.figure(figsize=(10, 10))

plt.plot(df['Epochs'],df['Train_Loss'],label='Training Loss')
plt.plot(df['Epochs'],df['Test_Loss'],label='Testing Loss')
...
df=pd.concat([df,pd.DataFrame(train_acc_list)],axis=1)
df=df.rename(columns={0:'Train_Accuracy'})

df=pd.concat([df,pd.DataFrame(test_acc_list)],axis=1)
df=df.rename(columns={0:'Test_Accuracy'})

plt.figure(figsize=(10, 10))

plt.plot(df['Epochs'],df['Train_Accuracy'],label='Training Accuracy')
plt.plot(df['Epochs'],df['Test_Accuracy'],label='Testing Accuracy')
